...
export default function CartPage() {
  const [items, setItems] = useState<CartItem[]>([
    { id: "3", name: "Industrial T-Slot Profile", model_number: "TS-4040-V2", price: 8.75, quantity: 50, stock: 1200, image_url: null },
  ])

  const updateQuantity = (id: string, delta: number) => {
    setItems((prev) => prev.map((item) => item.id === id ? { ...item, quantity: Math.max(1, Math.min(item.stock, item.quantity + delta)) } : item))
  }
