...
export function BuyerCatalogBrowser({ products, categoryData, wishlistedIds = [] }: { products: CatalogProduct[]; categoryData: CategoryData; wishlistedIds?: string[] }) {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((p) => {
      ...
      const matchesSearch = !search || p.name.toLowerCase().includes(search.toLowerCase()) || p.model_number.toLowerCase().includes(search.toLowerCase()) || p.category.toLowerCase().includes(search.toLowerCase()) || (p.description && p.description.toLowerCase().includes(search.toLowerCase()))
      ...
      {level === "products" && (
        <div className="space-y-4">
          ...
          <ProductSearchDropdown
            products={products.filter((p) => (!selectedCategory || p.main_category === selectedCategory) && (!selectedSubcategory || p.category === selectedSubcategory))}
            value={search}
            onChange={(v) => { setSearch(v); setCurrentPage(1) }}
            onSelect={(id) => { window.location.href = `/buyer/product/${id}` }}
