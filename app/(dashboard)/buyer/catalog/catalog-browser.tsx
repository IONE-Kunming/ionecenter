...
export function BuyerCatalogBrowser({ products, categoryData, wishlistedIds = [] }: { products: CatalogProduct[]; categoryData: CategoryData; wishlistedIds?: string[] }) {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((p) => {
      ...
      const matchesSearch = !search || p.name.toLowerCase().includes(search.toLowerCase()) || p.model_number.toLowerCase().includes(search.toLowerCase()) || p.main_category.toLowerCase().includes(search.toLowerCase()) || p.category.toLowerCase().includes(search.toLowerCase()) || (p.description && p.description.toLowerCase().includes(search.toLowerCase()))
      ...
      {level === "products" && (
        <div className="space-y-4">
          ...
          <ProductSearchDropdown
            ...
            onSelect={(p) => { window.location.href = `/buyer/product/${p.id}` }}
