...
export default function AdminBulkEditPage() {
  const [products, setProducts] = useState<{ id: string; name: string; model_number: string; price_per_meter: number; stock: number }[]>([])
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    getAllProducts().then((data: Product[]) => {
      setProducts(data.map((p) => ({ id: p.id, name: p.name, model_number: p.model_number, price_per_meter: p.price_per_meter, stock: p.stock })))
    })
  }, [])
