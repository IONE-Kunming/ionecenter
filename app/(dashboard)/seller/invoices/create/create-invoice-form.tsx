...
import { createOfflineInvoice, updateOfflineInvoice, searchSellerProducts, getSellerBankInfo, searchBuyers, getNextSellerInvoiceNumber, searchBuyerByCode, getSellerRecentBuyers, saveSellerBuyer, getSellerOrderBuyers } from "@/lib/actions/invoices"
...
function hasFoundBuyerBankInfo(buyer: FoundBuyer | null): boolean {
  if (!buyer) return false
  return !!(buyer.account_name || buyer.account_number || buyer.bank_name)
}
...
export function CreateOfflineInvoiceForm({ editData }: { editData?: EditData | null }) {
  ...
  const [myBuyersSearch, setMyBuyersSearch] = useState("")
  ...
  useEffect(() => {
    if (!showMyBuyers) return
    function handleClickOutside(e: MouseEvent) {
      if (myBuyersRef.current && !myBuyersRef.current.contains(e.target as Node)) {
  ...
  const handleMyBuyersToggle = useCallback(async () => {
    ...
    if (myBuyers.length === 0) {
      ...
      const buyers = await getSellerOrderBuyers()
      setMyBuyers(buyers)
      setMyBuyersLoading(false)
    }
  }, [showMyBuyers, myBuyers.length])
    ...
    if (buyer.user_code) {
      ...
      if (result) {
        setFoundBuyer({
          ...
          swift_code: result.swift_code ?? null,
          bank_name: result.bank_name ?? null,
          bank_region: result.bank_region ?? null,
          bank_code: result.bank_code ?? null,
          branch_code: result.branch_code ?? null,
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div ref={printRef} className="space-y-6 invoice-print-area" dir={isRtl ? "rtl" : undefined}>
        ...
        <div className="hidden print:block invoice-print-section">
          <h3 className="invoice-print-section-title">{lbl.buyerInformation}</h3>
          <div style={{ fontSize: "13px", lineHeight: "1.8" }}>
            <p><strong>{lbl.buyerCode}:</strong> {foundBuyer?.user_code || buyerCode || "—"}</p>
            <p><strong>{lbl.buyerNameLabel}:</strong> {foundBuyer?.display_name || buyerName || "—"}</p>
            <p><strong>{lbl.buyerEmailLabel}:</strong> {foundBuyer?.email || buyerEmail || "—"}</p>
          </div>
        </div>

        {/* Print-only: Seller Information Section */}
        <div className="hidden print:block invoice-print-section">
          <h3 className="invoice-print-section-title">{lbl.sellerInformation}</h3>
          <div style={{ fontSize: "13px", lineHeight: "1.8" }}>
            <p><strong>{lbl.sellerCode}:</strong> {bankInfo?.user_code || "—"}</p>
            <p><strong>{lbl.sellerName}:</strong> {bankInfo?.display_name || "—"}</p>
            <p><strong>{lbl.sellerEmail}:</strong> {bankInfo?.email || "—"}</p>
          </div>
        </div>
                ...
                <div className="space-y-2 print:hidden">
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="relative" ref={myBuyersRef}>
                      ...
                      {showMyBuyers && (
                        <div className="absolute z-20 mt-1 right-0 w-80 bg-background border rounded-md shadow-lg print:hidden">
                          <div className="p-2 border-b">
                            <Input
                              value={myBuyersSearch}
                              onChange={(e) => setMyBuyersSearch(e.target.value)}
