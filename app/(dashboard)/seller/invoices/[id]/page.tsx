...
import { PrintTrigger } from "../offline/[id]/print-trigger"
import { PrintButton } from "../offline/[id]/print-button"
...
export default async function SellerInvoiceDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ print?: string }> }) {
  const { id } = await params
  ...
  const hasBuyerBank = invoice.buyer_bank_account_name || invoice.buyer_bank_account_number || invoice.buyer_bank_name
      ...
      <div className="space-y-6 invoice-print-area">
        ...
        <div className="invoice-detail-section invoice-print-section">
          ...
          <div style={{ fontSize: "13px", lineHeight: "1.8" }}>
            <p className="text-foreground"><strong>{t("buyerCode")}:</strong> {invoice.buyer?.user_code || "—"}</p>
            <p className="text-foreground"><strong>{t("buyerName")}:</strong> {invoice.buyer?.display_name || invoice.buyer_name || "—"}</p>
            <p className="text-foreground"><strong>{t("buyerEmail")}:</strong> {invoice.buyer?.email || invoice.buyer_email || "—"}</p>
          </div>
        </div>
        ...
        {/* Buyer Bank Information (TT) — only if buyer bank fields are filled */}
        {hasBuyerBank && (
          <div className="invoice-detail-section invoice-print-section">
            <h3 className="invoice-detail-section-title invoice-print-section-title text-foreground">{t("buyerBankInformation")}</h3>
            <div style={{ fontSize: "13px", lineHeight: "1.8" }}>
              {invoice.buyer_bank_account_name && <p className="text-foreground"><strong>{t("accountHolder")}:</strong> {invoice.buyer_bank_account_name}</p>}
              {invoice.buyer_bank_account_number && <p className="text-foreground"><strong>{t("accountNumber")}:</strong> {invoice.buyer_bank_account_number}</p>}
              {invoice.buyer_bank_swift_code && <p className="text-foreground"><strong>{t("swiftBicCode")}:</strong> {invoice.buyer_bank_swift_code}</p>}
              {invoice.buyer_bank_name && <p className="text-foreground"><strong>{t("bankName")}:</strong> {invoice.buyer_bank_name}</p>}
              {invoice.buyer_bank_region && <p className="text-foreground"><strong>{t("bankRegion")}:</strong> {invoice.buyer_bank_region}</p>}
              {invoice.buyer_bank_code && <p className="text-foreground"><strong>{t("bankCode")}:</strong> {invoice.buyer_bank_code}</p>}
              {invoice.buyer_bank_branch_code && <p className="text-foreground"><strong>{t("branchCode")}:</strong> {invoice.buyer_bank_branch_code}</p>}
              {invoice.buyer_bank_address && <p className="text-foreground"><strong>{t("bankAddress")}:</strong> {invoice.buyer_bank_address}</p>}
            </div>
          </div>
        )}

        {/* Seller Bank Information — from seller profile */}
        {hasSellerBank && (
          <div className="invoice-detail-section invoice-print-section">
                ...
                <TableBody>
                  {invoice.items.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-right text-foreground">{formatCurrency(item.price, "USD", intlLocale)}</TableCell>
                      <TableCell className="text-right text-foreground">{item.quantity} {item.unit ?? ""}</TableCell>
                      <TableCell className="text-right font-medium text-foreground">{formatCurrency(item.subtotal, "USD", intlLocale)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <Card className="invoice-summary-card">
          <CardContent className="space-y-2 text-sm max-w-xs ml-auto pt-6">
            ...
            {invoice.deposit_paid > 0 && (
              <div className="flex justify-between text-green-600"><span>{t("amountPaid")}</span><span>{formatCurrency(invoice.deposit_paid, "USD", intlLocale)}</span></div>
            )}
            <div className="border-t pt-2 flex justify-between font-semibold text-lg invoice-amount-due"><span className="text-foreground">{t("amountDue")}</span><span className="text-foreground">{formatCurrency(invoice.remaining_balance, "USD", intlLocale)}</span></div>
          </CardContent>
        </Card>
        </div>
      </div>
